...
import { MutateMeasureReportMutationVariables } from 'iprocess-shared/graphql/API.js'
...
const parseEvent = (event: unknown) =>
  z.object({ arguments: objectOf(EmptyZodObject).withType<MutateMeasureReportMutationVariables>() }).parse(event)
